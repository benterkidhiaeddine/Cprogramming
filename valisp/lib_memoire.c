...
#pragma weak tableau_symboles_init
void tableau_symboles_init(void);


#pragma weak tableau_symboles_ajout
void tableau_symboles_ajout(sexpr s);
...
int charger_image(char *nom_fichier) {
    ...
    fclose(fichier);
    if (tableau_symboles_init) tableau_symboles_init();
    for (i=0; i != (MEMOIRE_DYNAMIQUE[i]&0x7FFF); i = (MEMOIRE_DYNAMIQUE[i]&0x7FFF)) {
        ...
        if ((MEMOIRE_DYNAMIQUE[i]>>15) & 1 ) {
            ...
            if (tableau_symboles_ajout &&
                usage_bloc(i)          &&
                symbol_p(&MEMOIRE_DYNAMIQUE[i+1])) {
                tableau_symboles_ajout(&MEMOIRE_DYNAMIQUE[i+1]);
            }
        }
    }
    return 0;
...
int comparer_image_occupee(void) {
    int i, j, suivant;
    for (i=0; i != (MEMOIRE_DYNAMIQUE[i]&0x7FFF); i = (MEMOIRE_DYNAMIQUE[i]&0x7FFF)) {
        suivant = ((MEMOIRE_DYNAMIQUE[i]>>15) & 1 ) ? MEMOIRE_DYNAMIQUE[i]&0x7FFF : i+1;
        for (j=i; j<suivant ; j++) {
            if (MEMOIRE_COPIE[i] != MEMOIRE_DYNAMIQUE[i]) {
                printf("[debug] i=%d\n", i);
                return -i;
